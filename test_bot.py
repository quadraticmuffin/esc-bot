...
async def collect_composers(ctx, member):
    ...
    try:
        ans = await client.wait_for('message', timeout=20, check = lambda m: m.author.id == member.id and m.channel == member.dm_channel)
...
@is_jett() # TODO CHECK if shwain can broadcast
async def broadcast(ctx):
    guild = discord.utils.find(lambda g: g.name == GUILD, client.guilds)
    for channel in guild.text_channels:
        await channel.send(ctx.message.content[10:])
